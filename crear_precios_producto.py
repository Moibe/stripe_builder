...
def crear_precio_stripe(product_id, monto, moneda, nombre_pais):
    ...
    try:
        ...
        precio = stripe.Price.create(
            ...
            nickname=f"{nombre_pais} - Acceso para siempre"
        )
...
def main():
    print("=" * 90)
    print("🌍 CREAR PRECIOS POR PAÍS - PRODUCTO: Acceso para siempre")
    print("=" * 90)
    ...
    if precios_creados:
        ...
        archivo_salida = f"precios_Acceso_para_siempre_{timestamp}.txt"
        ...
        with open(archivo_salida, 'w', encoding='utf-8') as f:
            f.write("PRECIOS CREADOS - ACCESO PARA SIEMPRE\n")
            f.write("=" * 70 + "\n")
            f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Producto: Acceso para siempre\n")
