...
PRECIO_MXN = 100
NOMBRE_PRODUCTO = "Map Location Full Real Time"
AMBIENTE = "prod"
...
def main():
    ...
    if precios_creados:
        ...
        print(f"\n📋 PRIMEROS 10 PRECIOS:")
        for i, precio in enumerate(precios_creados[:10], 1):
            print(f"   {i:2}. {precio['pais']:25} ${precio['monto']:8.2f} {precio['moneda']} → {precio['price_id']}")
        if len(precios_creados) > 10:
            print(f"   ... y {len(precios_creados) - 10} más")
    else:
